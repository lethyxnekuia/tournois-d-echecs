...
from controllers import player_controller, main_controller
...
class TournamentMenuController:
    ...
    def tournament_choice(self,tournaments):
        for count, tournament in enumerate(tournaments, start=1) :
                self.tounament_choice_menu[f"{count}"] =  {"label" : tournament.tournament_name,"id" : tournament.tournament_id}
    ...
    def create_a_round(self):
        ...
        for i in range(players_count/2) :
            ...
            while count != 0 :
                potential_player = self.potential_player(player_one["player"], sorted_players_list_by_score[count]["player"], tournament.rounds)
                if potential_player == False :                     
                    count = count + 1

                else:
                    potential_choice_list.append(sorted_players_list_by_score[count])
                    if sorted_players_list_by_score[count]["tournament_score"] == sorted_players_list_by_score[count + 1]["tournament_score"]:
                        count = count + 1
